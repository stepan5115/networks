...
void parse_args(int argc, char* argv[]) {
    for (int i = 1; i < argc; i++) {
        ...
        if (arg.find("--delay=") == 0) {
            ...
            std::cerr << "Usage: " << argv[0] << " [--delay=ms] [--drop=probability] [--corrupt=probability]" << std::endl;
        }
    }
}
...
bool simulate_network_issues(MessageEx& msg, int client_sock) {
    if (network_delay_ms > 0) {
        log_layer(3, "[SIM] DELAY applied: " + std::to_string(network_delay_ms) + " ms");
    ...
    if (drop_probability > 0.0) {
        ...
        if (random < drop_probability) {
            log_layer(3, "[SIM] DROP (id=" + std::to_string(msg.msg_id) + 
                      ", rate=" + std::to_string(drop_probability) + ")");
            return false;
        }
    }
    ...
    if (corrupt_probability > 0.0) {
        ...
        if (random < corrupt_probability) {
            ...
            if (payload_len > 0) {
                ...
                log_layer(3, "[SIM] CORRUPT payload (id=" + std::to_string(msg.msg_id) + 
                      ", pos=" + std::to_string(pos) + 
                      ", original=" + std::to_string((int)(unsigned char)original) + 
                      " -> " + std::to_string((int)(unsigned char)msg.payload[pos]) + ")");
            }
        }
    }
...
bool recv_message(int socket, MessageEx& msg, bool& dropped) {
    dropped = false;
    ...
    if (!simulate_network_issues(msg, socket)) {
        dropped = true;
        return true;
    }
...
void* handle_client(int client_sock) {
    ...
    if (!recv_message(client_sock, msg, dropped) || dropped || msg.type != MSG_AUTH) {
    ...
    while (keepRunning && client.active) {
        dropped = false;
        if (!recv_message(client_sock, msg, dropped)) {
